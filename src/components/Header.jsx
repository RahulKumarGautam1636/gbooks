...
import { compToggled, loaderToggled, sideBarToggled } from "../slices";
import { getUrl, logOut } from "./utils/utilities";
...
const Header = ({ setOffset }) => {
    ...
    useEffect(() => {
        if (!compCode) return alert('No company code recieved');
        const getData = async () => {
            dispatch(loaderToggled(true));                     
            let compData = await getUrl(`/api/CompMast?CID=${compCode}`);
            dispatch(loaderToggled(false));
            if (!compData) return;
            dispatch(compToggled(compData));
        }
        getData();
                ...
                <ul className="header-nav">
                    <li>
                        <div className="toggler-box d-flex align-items-center">
                            ...
                            <img src={`https://erp.gsterpsoft.com/Content/CompanyLogo/${company.LogoUrl}`} alt="Logo" style={{maxHeight: '2.4em', margin: '0 0.8em'}}/>
                            <h3 className="compName">{company.COMPNAME}</h3>
                        </div>
                    </li>
