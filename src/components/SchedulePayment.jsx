...
import { SCHEDULE_BILL, getUrl } from "./utils/utilities";
...
const SchedulePayment = () => {
    ...
    useEffect(() => {
        ...
        const getData = async () => {
            ...
            let res = await getUrl(`/api/PGPayment?PartyCode=${user.PartyCode}&CID=${compCode}&LOCID=${user.LocationId}&FID=10`);
                                ...
                                {data.DirectSalesCollection.map(i => (
                                    <tr key={i.BillId}>
                                        ...
                                        <td className="text-center">
                                            ...
                                            <Link to={`/paymentEntry/${i.BillId}/${SCHEDULE_BILL}`}><i className="bx bx-money del-icon" onClick={() => handlePay(i.BillId)} role="button" title="Pay Now" style={{fontSize: '1.6em', verticalAlign: 'middle', color: '#1ab500'}}></i></Link>
                                        </td>
                                        {/* <td className="text-center">
